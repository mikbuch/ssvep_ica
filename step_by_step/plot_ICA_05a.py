...
In this script I introduce a function to perform automated components removal
using ICA. It writes results to the file for later comparisons.

Clear the data (remove noise components).
Show timecourse of the chosen electrodes before and after noise removal.

ICA for choosen electrodes only.
...
# picks_ssvep = [9, 14, 20, 22, 27, 38, 80]
# picks_ssvep = [80, 79, 92, 93, 82, 71, 35, 119]

picks_ssvep = [7, 8, 9, 14, 15, 16, 20, 21, 22, 27, 28, 29, 36, 37, 38]

output_dir = '/tmp'


def clean_data(data_dir, data_filename, picks=None,
               output_dir='./', output_delimiter='\t'):
    ...
    if picks is None:
        ...
        picks_chs = [name for (i, name) in enumerate(raw.ch_names)
                     if i in picks]

        # If no picks specified, take all electrodes.
        picks_eeg = mne.pick_types(raw.info, meg=False, eeg=True, eog=False,
                                   stim=False, exclude='bads',
                                   )
                                   # selection=picks_chs)
    else:
        picks_eeg = np.array(picks)
...
clean_data(data_dir=data_dir, data_filename=data_filename, picks=picks_ssvep,
