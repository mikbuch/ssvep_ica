...
raw_basic = mne.io.read_raw_fif(raw_fname, preload=True, add_eeg_ref=False)
picks_meg_basic = mne.pick_types(raw_basic.info, meg=True, eeg=False, eog=False,
                           stim=False, exclude='bads')

raw_basic.filter(1, 40, n_jobs=2)
...
var_name = 'EEGdata'
ch_names_filepath = '../example_data/128_Biosemi_names.txt'
raw = create_MNE_Raw(data_filepath, var_name, ch_names_filepath, sfreq=256)
...
ica_basic = ICA(n_components=n_components, method=method, random_state=random_state)
...
ica_basic.fit(raw_basic, picks=picks_meg_basic, decim=decim, reject=reject)
